## Introduction

## État de la question
<!--
Présenter notre projet, la problématique

- Il y a un problème dans la relation entre la représentation de la francophonie au Québec et les méthodes utilisées par le milieu culturel pour réduire l'attrition du contenu culturel francophone \--\> plateformes.
    - C'est un enjeu qui mobilise la pensée politique et sociale : est-ce qu'on veut utiliser des fonds publiques ?
    - Il y a les médias étatiques, mais la consommation de culture ne passe pas par là, mais plutôt par les GAFAM : l'état désinvestit la culture
    - Le rapport des GAFAM à la culture québécoise est un rapport pécunier : c'est un rapport à la rétention.
    - La production d'un espace concurrentiel n'est pas une entreprise concluante : la rétention produite par la société d'état n'est pas intéressante
    - L'école, les espaces publiques, les insitutions comme des espaces de médiation culturels, hors des plateformes

- Ce qu'on défend, c'est que
    - les curateur·ices sont chaque fois racheté·es (parfois des problèmes de droits, cf 8tracks, cf QUB musique)
        - MusiQC : un geste conscient de la manière dont on va chercher de la musique, ce qui va à l'encontre de la manière dont l'écosystème fonctionne actuellement. 
    - est-ce que c'est la meilleure manière de partager de la culture
    - les bibliothécaires ont un rôle dans cet écosystème; iels font partie de la réponse

- Métamusique : un contrôle sur les métadonnées par les artistes. Cependant, les métadonnées sont lacunaires ou absentes. On veut jouer sur le terrain de la données : des métadonnées plus riches permetteraient aux plateformes de ramasser plus de musique québécoise.

- Loi n°109 : projet de loi pour modifier la charte des droits et libertés pour ajouter le « droit à la découvrabilité des contenus culturels » et protéger l'accès à des contenus francophones. Les plateformes numériques et les fabricants doivent avoir une interface en français. YouTube, Spotify, Amazon et Apple Music considèrent que cela « limite la liberté d'écoute des auteur·ices ».
-->

## Méthodologie

<!--
Petit sommaire de nos recherches
-->

## Présentation de la visualisation

### Examen de la visualisation
<!---->

MusiQC : en l'espace de moins d'un an, le contenu québécois est noyé dans le contenu « francophone », donc français. Musique « en français » n'est pas un regroupement fédérateur, n'a pas d'impact sur la désirabilité des contenus, ne convient qu'à ceux et celles qui le cherchent déjà.

### Un cas particulier : _La Hit Liste_

_La Hit Liste_ est une liste de lecture qui mélange des chansons populaires à l'échelle mondiale et des chansons populaire québécoises. C'est une approche qui ressemble à celle adoptée par les radios populaire : utiliser du contenu populaire pour valoriser le contenu québécois.

## Perspective pour les bibliothécaires
<!-- On pourrait présenter ici les ressources intéressantes pour la découverte de musique par des bibliothécaires en contexte québécois :
* MusiQC
* MusiCat
* Metamusique.ca (dépôt légal de musique)

Mais aussi pour que les bibliothécaires se gardent au courant des artistes de la scène locale
* sorstu.ca -->

<!--
Donner des ressources, proposer des plans d'action
-->

- Le rôle des bibliothécaires : gérer un type d'accès à l'information
- Les métadonnées ; bibliothécaire de données, les artistes ne sont pas formé·es pour informer les métadonnées, mettre leurs contenus en relation : les bibliothécaires, oui.